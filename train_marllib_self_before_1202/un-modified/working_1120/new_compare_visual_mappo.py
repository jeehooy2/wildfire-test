"""
MARLlib 학습된 정책과 랜덤 정책 시각적 비교 (이질적 에이전트 지원)

MARLlib MAPPO로 학습된 이질적 에이전트(헬리콥터, 트럭, 승무원)와
...
실행 예시:
# 방법 1: 디렉토리만 지정 (자동으로 최신 checkpoint 선택) - 권장
python train_marllib_self/new_compare_visual.py \
    --checkpoint train_marllib_self/experiments/mappo/run4 \
    --episodes 3 \
    --seed 42

# 방법 2: 전체 checkpoint 경로 직접 지정
python train_marllib_self/new_compare_visual.py \
...
# 방법 3: 출력 디렉토리 직접 지정
python train_marllib_self/new_compare_visual.py --checkpoint train_marllib_self/experiments/mappo/run3 --output-dir ./my_gifs/
"""
...
# ============================================================================
# [이전 코드 - 단순 MLP] 주석처리 - checkpoint 로드 실패
# ============================================================================
...
# ============================================================================
# [새로운 코드 - MARLlib CentralizedCriticMLP 아키텍처]
# ============================================================================
def create_policy_network(obs_dim, action_dim):
    """
...
def main(checkpoint_path, num_episodes=3, seed=42, output_dir=None):
    ...
    # ============================================================================
    # [새로운 코드 - share_policy='group' 지원, 에이전트 객체 ID]
    # ============================================================================
    # 에이전트 타입과 수 파악
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="MARLlib 체크포인트 경로. 디렉토리(run 폴더)를 지정하면 자동으로 최신 checkpoint를 선택합니다. "
             ...
             "예시 2: train_marllib_self/experiments/mappo/run1/mappo_mlp_wildfire-ma/.../checkpoint_000050"
    )
    parser.add_argument(
        "--episodes",
        type=int,
