...
class AgentState(enum.IntEnum):
    """Agent states for activity and recharging (supply source return mechanism)"""
    ACTIVE = 0         # 활동 중 (불 진압)
...
class WildfireActions(enum.IntEnum):
    ...
    Actions 1-8 are ordered clockwise starting from NORTH:
    NORTH(1) -> NORTH_EAST(2) -> EAST(3) -> SOUTH_EAST(4) ->
    SOUTH(5) -> SOUTH_WEST(6) -> WEST(7) -> NORTH_WEST(8)
...
class Agent(WorldObj):
    ...
    def __init__(
        ...
        # Phase 3/4: 급수원 반환 및 재충전 메커니즘
        self.home_pos = None                            # 급수원(홈베이스) 위치
    ...
    def north_east_pos(self):
        """
        Get the position of the cell to the north-east of the agent

        Returns
        -------
        NDArray
            the position of the cell to the north-east of the agent
        """
        if self.pos is None:
            raise ValueError("Agent position is not set")
        else:
            return self.pos + np.array([1, -1])

    def north_west_pos(self):
        """
        Get the position of the cell to the north-west of the agent

        Returns
        -------
        NDArray
            the position of the cell to the north-west of the agent
        """
        if self.pos is None:
            raise ValueError("Agent position is not set")
        else:
    ...
    def south_east_pos(self):
        """
        Get the position of the cell to the south-east of the agent

        Returns
        -------
        NDArray
            the position of the cell to the south-east of the agent
        """
        if self.pos is None:
            raise ValueError("Agent position is not set")
        else:
    ...
    def south_west_pos(self):
        """
        Get the position of the cell to the south-west of the agent

        Returns
        -------
        NDArray
            the position of the cell to the south-west of the agent
        """
        if self.pos is None:
            raise ValueError("Agent position is not set")
        else:
...
class Helicopter(Agent):
    """
    Helicopter agent for wildfire management.
    Helicopters have enhanced view range, can fly over obstacles, and have high speed.
    """
    ...
    def __init__(
        ...
        """Initialize the Helicopter agent
        ...
        index : int, optional
            a number useful to identify the instantiated agent, by default 0
        view_size : int, optional
            the size of agent view (larger for helicopters), by default 10
        actions : Type[ActionsT], optional
        ...
        self.max_active_time = 50              # 200 스텝 활동 가능
        self.recharge_time = 10                 # 15 스텝만 충전
    ...
    def render(self, img):
        ...
        """
        c = self.world.COLORS[self.color]
        # Render Helicopter as a smaller triangle with a circle to represent aerial vehicle
        tri_fn = point_in_triangle(
            (0.15, 0.25),
            (0.85, 0.50),
            (0.15, 0.75),
...
class Truck(Agent):
    """
    Fire truck agent for wildfire management.
    Trucks have standard view range and moderate speed.
    """
    ...
    def __init__(
        ...
        self.recharge_time = 10                 # 20 스텝 충전 (기본값)
    ...
    def render(self, img):
        ...
        """
        c = self.world.COLORS[self.color]
        # Render truck as a larger, more robust triangle
        tri_fn = point_in_triangle(
            (0.10, 0.15),
            (0.90, 0.50),
            (0.10, 0.85),
...
class Crew(Agent):
    ...
    Crew members have standard view range, low speed, but lower fire suppression effectiveness.
    """
    ...
    def __init__(
        ...
        # 급수원 반환 시간 커스터마이징 (Crew는 느려서 자주 돌아와야 함)
        self.max_active_time = 100              # 100 스텝 활동 가능 (짧음)
        self.recharge_time = 25                 # 25 스텝 충전
