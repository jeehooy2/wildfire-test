...
def cooperative_reward(
    ...
    # agents_contributed = sum(1 for v in agent_tree_extinguished.values() if v == 1)
    agents_on_fire = sum(1 for v in agent_on_fire_tree.values() if v == 1)
    
    shared_reward = 2.0 * T_e + 0.5 * agents_on_fire - 0.3 * T_n - 0.5 * T_b
    ...
    for i in range(num_agents):
        ...
        individual_reward = 2.0 * extinguished_fire + 0.5 * on_fire

        # Combined reward: individual + shared component
        rewards[str(i)] = 0.7 * shared_reward  + 0.3 * individual_reward
