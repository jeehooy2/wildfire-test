"""
RLlib MultiAgentEnv 래퍼 (글로벌 상태 지원)

wildfire-v0 환경을 RLlib의 MultiAgentEnv 형식으로 변환
이 버전은 MARLlib의 중앙화된 비평자(centralized critic)를 지원하기 위해
관찰 공간을 {'state': ..., 'obs': ...} 구조로 변환합니다.
"""
...
class WildfireRLlibEnv(MultiAgentEnv):
    """RLlib용 Wildfire 다중 에이전트 환경 래퍼 (글로벌 상태 지원)"""
    ...
    def __init__(self, env_config):
        ...
        print(f"\n=== WildfireRLlibEnv 초기화 (글로벌 상태 지원) ===")
        ...
        # ===== 글로벌 상태 지원을 위한 수정 시작 =====
        # MARLlib의 centralized_critic에서 요구하는 구조
        # observation_space: Dict with 'state' (global) and 'obs' (local) keys
        from gym.spaces import Dict as GymDict

        # 글로벌 상태 공간: 실제 환경의 get_state()에서 반환되는 벡터
        # get_state()는 (obs_depth + 1) * grid_size * grid_size + 1 크기의 flattened 배열 반환
        obs_depth = self.env.obs_depth
        grid_size = self.env.grid_size
        global_state_size = (obs_depth + 1) * grid_size * grid_size + 1

        global_state_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
        ...
        # 관찰 공간: 'state' (글로벌) + 'obs' (로컬)
        self._observation_space = GymDict({
            "state": global_state_space,
            "obs": single_obs_space
        })
        # ===== 글로벌 상태 지원을 위한 수정 끝 =====

        self._action_space = single_action_space

        print(f"RLlib 래퍼 생성 (글로벌 상태 지원):")
        print(f"  에이전트 수: {self.num_agents}")
        print(f"  액션 공간: {single_action_space}")
        print(f"  로컬 관찰 공간: {single_obs_space}")
        print(f"  글로벌 상태 공간: {global_state_space}")
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # 글로벌 상태: 환경의 get_state() 메서드 사용 (모든 에이전트가 공유)
        global_state = np.asarray(self.env.get_state(), dtype=np.float32)

        # 각 에이전트의 관찰을 {'obs': local_obs, 'state': global_state} 형태로 반환
        # RLlib Dict는 키를 알파벳순으로 정렬하므로 'obs'가 'state'보다 앞
        for idx, agent_id in enumerate(self.agents):
            if str(idx) in obs_dict:
                ...
                local_obs = np.asarray(obs_dict[str(idx)], dtype=np.float32)
                obs[agent_id] = {
                    "obs": local_obs,
                    "state": global_state  # 모든 에이전트가 동일한 global_state 공유
                }
    ...
    def step(self, action_dict):
        ...
        # 결과를 RLlib 형식으로 변환 (프리픽스 ID로 다시 매핑, 글로벌 상태 포함)
        # RLlib Dict는 키를 알파벳순으로 정렬하므로 'obs'가 'state'보다 앞
        observations = {}
        rewards = {}
        infos = {}
    ...
    def observation_space_sample(self, agent_ids=None):
        ...
        # 글로벌 상태 샘플링 (모든 에이전트가 공유)
        global_state = self._observation_space["state"].sample()

        return {agent_id: {
            "obs": self._single_obs_space.sample(),  # 각 에이전트별 다른 obs
            "state": global_state  # 모든 에이전트가 동일한 global_state
