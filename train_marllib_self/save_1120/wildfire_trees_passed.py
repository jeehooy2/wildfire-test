...
class WildfireEnv(MultiGridEnv):
    ...
    def step(self, actions):
        ...
        # 에이전트별로 경로상의 trees를 추적하기 위한 딕셔너리
        # 각 에이전트가 이동 중 지나간 trees (시작점, 도착점 제외)
            ...
            for path_tree in path_trees:
                if path_tree.state == 1:  # on fire 상태
                    ...
                    # on fire -> healthy
                        ...
                        if path_tree not in trees_to_healthy_state:
                            trees_to_healthy_state.append(path_tree)
                            self.trees_on_fire -= 1
                            if self.log_selfish_region_metrics and path_tree.region != "common":
                                self.selfish_region_trees_on_fire[int(path_tree.region)] -= 1
        ...
        if self.trees_on_fire == 0:
            ...
            if self.reward_function is not None:
                ...
                # agent_tree_extinguished: 각 에이전트가 있는 나무나 경로상의 나무가 진화되었는지 (0 or 1)
