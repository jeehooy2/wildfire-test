"""
MARLlib PPO 학습된 정책과 랜덤 정책 시각적 비교 (PPO 전용)
...
def main(checkpoint_path, num_episodes=3, seed=42, output_dir=None):
    """
    메인 함수: 학습된 모델과 랜덤 정책을 시각적으로 비교 (PPO용, 이질적 에이전트)
    ...
    print("=" * 80)
    print("MARLlib PPO 학습된 정책 vs 랜덤 정책 시각적 비교")
    print("=" * 80)
    print(f"\n✓ 사용된 Checkpoint:")
    ...
    with open(stats_path, 'w') as f:
        f.write("Trained vs Random Agent Visualization Stats (PPO - Decentralized)\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"Checkpoint: {checkpoint_path}\n")
        f.write(f"Algorithm: PPO (no centralized critic)\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MARLlib PPO 학습된 정책과 랜덤 정책 시각적 비교")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
