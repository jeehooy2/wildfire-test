"""
Train PPO agents on wildfire suppression using new_wrapper.py

이 스크립트는 add_new_env.py의 구조를 따라서 new_wrapper.py를 사용하여
wildfire 환경에서 PPO 에이전트를 학습시킵니다.



실행:
python train_marllib_self/new_train_ppo.py --run-name run1
"""
...
# Auto-generated by new_train_ppo.py
...
if __name__ == '__main__':
    # 커맨드라인 인자 파싱 (선택적)
    parser = argparse.ArgumentParser(description='Train PPO on Wildfire Environment')
    ...
    # 출력 디렉토리 설정
    output_dir = Path(__file__).parent / "experiments" / "ppo" / run_name
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("Wildfire PPO Training with new_wrapper")
    print("=" * 80)
    print(f"\nRun name: {run_name}")
    ...
    # MARLlib config에 필수 설정 추가 (ppo() 호출 후에 수정)
    if ppo.config_dict is not None:
        ppo.config_dict["global_state_flag"] = False
        ppo.config_dict["local_dir"] = str(output_dir)  # 커스텀 출력 경로
    else:
