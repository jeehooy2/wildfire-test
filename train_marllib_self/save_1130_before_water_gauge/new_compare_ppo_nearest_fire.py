...
# ============================================================================
# [Policy Network 생성 (고정된 2-layer architecture)]
# ============================================================================
def create_policy_network(obs_dim, action_dim, hidden_dim=256):
    """
    MARLlib의 고정된 2-layer encoder 구조를 가진 policy network 생성

    다양한 input dimension을 가진 checkpoint와 호환성이 있습니다.
    ...
    class PolicyNetwork(torch.nn.Module):
        def __init__(self, obs_dim, action_dim, hidden_dim=256):
            ...
            # Policy encoder (2 layers with dynamic input dimension)
            ...
            # Value function encoder (2 layers)
        ...
        def get_action(self, obs, target_obs_dim=None):
            """
            Select action from observation

            If observation dimension doesn't match network input,
            pads or truncates the observation to match.
            """
            with torch.no_grad():
                obs_array = np.asarray(obs)

                # 관찰 차원이 네트워크 입력과 다르면 조정
                if target_obs_dim is not None and len(obs_array) != target_obs_dim:
                    if len(obs_array) < target_obs_dim:
                        # Zero-pad to target dimension
                        padded_obs = np.zeros(target_obs_dim)
                        padded_obs[:len(obs_array)] = obs_array
                        obs_array = padded_obs
                    else:
                        # Truncate to target dimension
                        obs_array = obs_array[:target_obs_dim]
...
def load_marllib_weights_to_network(network, marllib_weights):
    """
    MARLlib checkpoint 가중치를 고정된 2-layer network로 로드

    처음 2개의 encoder layer만 로드합니다.
    """
    weight_mapping = {
...
def main(checkpoint_path, num_episodes=3, seed=42, output_dir=None):
    ...
    # 행동 공간 차원 파악 (관찰 공간은 checkpoint에서 추출)
    sample_agent_id = list(env_ppo.action_space.keys())[0]
    action_dim = env_ppo.action_space[sample_agent_id].n
    ...
    try:
        ...
        if policies_weights is not None:
            # 첫 번째 정책에서 obs_dim 추출
            first_policy_name = list(policies_weights.keys())[0]
            first_weights = policies_weights[first_policy_name]

            # checkpoint의 실제 input dimension 파악
            if 'p_encoder.encoder.0._model.0.weight' in first_weights:
                weight_shape = first_weights['p_encoder.encoder.0._model.0.weight'].shape
                checkpoint_obs_dim = weight_shape[1]  # (out_dim, in_dim)
                print(f"  ✓ Checkpoint에서 관찰 공간 차원 감지: {checkpoint_obs_dim}")
            else:
                # fallback: 환경에서 파악
                checkpoint_obs_dim = env_ppo.observation_space[sample_agent_id].shape[0]
                print(f"  ⚠ Checkpoint weight에서 obs_dim 감지 실패, 환경 설정에서 사용: {checkpoint_obs_dim}")

            print(f"  행동 공간 차원: {action_dim}")
            ...
            if 'shared' in policies_weights:
                ...
                print("\n  [Shared Policy 로드 중]")
                policy_networks['shared'] = create_policy_network(checkpoint_obs_dim, action_dim, hidden_dim=256)
                ...
                for policy_name in ['helicopter_policy', 'truck_policy', 'crew_policy']:
                    if policy_name in policies_weights:
                        print(f"\n  [{policy_name} 로드 중]")
                        policy_networks[policy_name] = create_policy_network(checkpoint_obs_dim, action_dim, hidden_dim=256)
