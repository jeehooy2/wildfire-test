...
class WildfireRLlibEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config):
        ...
        # 글로벌 상태 공간: 모든 에이전트 정보를 포함하는 평탄화된 벡터
        # 간단하게 모든 에이전트 관찰의 합산 차원을 사용
        obs_size = single_obs_space.shape[0] if len(single_obs_space.shape) > 0 else 1
        global_state_size = obs_size * self._num_agents
    ...
    def reset(self, *, seed=None, options=None):
        ...
        for idx, agent_id in enumerate(self.agents):
            if str(idx) in obs_dict:
                ...
                obs_arrays_by_idx[idx] = obs_array
                global_state_parts.append(obs_array)

        # 글로벌 상태: 모든 에이전트 관찰을 연결한 벡터
        global_state = np.concatenate(global_state_parts, axis=0).astype(np.float32)

        # 각 에이전트의 관찰을 {'state': global_state, 'obs': local_obs} 형태로 반환
        for idx, agent_id in enumerate(self.agents):
            if idx in obs_arrays_by_idx:
                obs[agent_id] = {
                    "state": global_state,
                    "obs": obs_arrays_by_idx[idx]
                }
    ...
    def observation_space_sample(self, agent_ids=None):
        ...
        local_obs = self._single_obs_space.sample()

        return {agent_id: {"state": global_state, "obs": local_obs} for agent_id in agent_ids}
