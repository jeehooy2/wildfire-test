"""
MARLlib 학습된 정책과 랜덤 정책 시각적 비교 (이질적 에이전트 지원, 모든 알고리즘 호환)
...
실행 예시:
# MAPPO: experiments/mappo/run1에서 결과를 results/mappo/run1에 저장
python train_marllib_self/new_compare_visual_all_algo.py \
    --checkpoint train_marllib_self/experiments/mappo/run1 \
    --episodes 3 \
    --seed 42

# MAA2C: experiments/maa2c/run1에서 결과를 results/maa2c/run1에 저장
python train_marllib_self/new_compare_visual_all_algo.py \
    --checkpoint train_marllib_self/experiments/maa2c/run1 \
    --episodes 3 \
    --seed 42

# 커스텀 출력 디렉토리 지정
python train_marllib_self/new_compare_visual_all_algo.py \
    --checkpoint train_marllib_self/experiments/mappo/run3 \
    --output-dir ./my_gifs/
"""
...
# ============================================================================
# [MARLlib CentralizedCriticMLP 아키텍처]
# ============================================================================
def create_policy_network(obs_dim, action_dim):
    ...
    이 구조는 checkpoint의 policy_helicopter_, policy_truck_ 키 아래에
    위의 가중치들이 저장되어 있는 것과 정확히 일치합니다.
    """
    ...
    class PolicyNetwork(torch.nn.Module):
        def __init__(self, obs_dim, action_dim, hidden_dim=256):
            ...
            # Layer 0: obs_dim -> hidden_dim (주의: checkpoint에서는 23820으로 저장되어 있을 수 있음)
...
def load_marllib_weights_to_network(network, marllib_weights):
    ...
    MARLlib 가중치 구조:
      {
        'p_encoder.encoder.0._model.0.weight': (256, obs_dim),
        'p_encoder.encoder.0._model.0.bias': (256,),
        'p_encoder.encoder.1._model.0.weight': (256, 256),
        'p_encoder.encoder.1._model.0.bias': (256,),
        'p_branch._model.0.weight': (action_dim, 256),
        'p_branch._model.0.bias': (action_dim,),
        'vf_encoder.encoder.0._model.0.weight': (256, obs_dim),
        'vf_encoder.encoder.0._model.0.bias': (256,),
        'vf_encoder.encoder.1._model.0.weight': (256, 256),
        'vf_encoder.encoder.1._model.0.bias': (256,),
        'vf_branch._model.0.weight': (1, 256),
        'vf_branch._model.0.bias': (1,),
        'cc_vf_encoder.encoder.0._model.0.weight': (256, obs_dim or 23820),
        'cc_vf_encoder.encoder.0._model.0.bias': (256,),
        'cc_vf_encoder.encoder.1._model.0.weight': (256, 256),
        'cc_vf_encoder.encoder.1._model.0.bias': (256,),
        'cc_vf_branch._model.0.weight': (1, 256),
        'cc_vf_branch._model.0.bias': (1,),
      }
...
def main(checkpoint_path, num_episodes=3, seed=42, output_dir=None):
    """
    메인 함수: 학습된 모델과 랜덤 정책을 시각적으로 비교 (이질적 에이전트 지원)
    ...
    if output_dir is None:
        # 경로 예: train_marllib_self/experiments/mappo/run1/... -> mappo, run1
        # 또는: train_marllib_self/experiments/maa2c/run1/... -> maa2c, run1
    ...
    print("=" * 80)
    print("MARLlib 학습된 정책 vs 랜덤 정책 시각적 비교 (모든 알고리즘 호환)")
    print("=" * 80)
    print(f"\n✓ 사용된 Checkpoint:")
    ...
    with open(stats_path, 'w') as f:
        f.write("Trained vs Random Agent Visualization Stats (Heterogeneous Agents)\n")
        f.write("=" * 80 + "\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MARLlib 학습된 정책과 랜덤 정책 시각적 비교 (모든 알고리즘 호환)")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
        help="MARLlib 체크포인트 경로. 디렉토리(run 폴더)를 지정하면 자동으로 최신 checkpoint를 선택합니다. "
             ...
             "예시 3: train_marllib_self/experiments/mappo/run1/mappo_mlp_wildfire-ma/.../checkpoint_000050"
    )
    parser.add_argument(
        "--episodes",
        type=int,
