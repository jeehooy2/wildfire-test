"""
Train MAA2C agents on wildfire suppression using new_wrapper.py

이 스크립트는 add_new_env.py의 구조를 따라서 new_wrapper.py를 사용하여
wildfire 환경에서 MAA2C 에이전트를 학습시킵니다.



실행:
python train_marllib_self/new_train_maa2c.py --run-name run1
"""
...
# Auto-generated by new_train_maa2c.py
...
if __name__ == '__main__':
    # 커맨드라인 인자 파싱 (선택적)
    parser = argparse.ArgumentParser(description='Train MAA2C on Wildfire Environment')
    ...
    if args.run_name:
        ...
        run_name = f"maa2c_{timestamp}"

    # 출력 디렉토리 설정
    output_dir = Path(__file__).parent / "experiments" / "maa2c" / run_name
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("Wildfire MAA2C Training with new_wrapper")
    print("=" * 80)
    print(f"\nRun name: {run_name}")
    ...
    # MAA2C 알고리즘 선택
    print(f"MAA2C 알고리즘 초기화 중...")
    maa2c = marl.algos.maa2c(
        hyperparam_source="common",
        batch_episode=10,
        lr=0.0005,        # 학습률 조정
    )

    # wildfire_maa2c_config = {
    #   # 핵심 학습 파라미터
    #   "use_gae": True,              # GAE 사용 (안정적 학습)
    #   "lambda": 0.95,               # 0.95 추천 (1.0보다 약간 더 낮추어 variance 감소)
    #   "vf_loss_coeff": 1.0,         # 기본값 유지

    #   # 배치 및 학습률
    #   "batch_episode": 32,          # MPE(128)와 SMAC(10)의 중간값
    #                                 # 긴 에피소드를 고려하여 적당히 증가
    #   "batch_mode": "complete_episodes",  # 완전한 에피소드 사용
    #   "lr": 0.0003,                 # MPE 기반 (복잡성이 높으므로)

    #   # 탐색 파라미터
    #   "entropy_coeff": 0.02,        # 0.01 → 0.02로 상향 (탐색 강화)
    #                                 # 큰 상태공간에서 충분한 탐색 필요
    # }
