...
class MultiGridEnv(gym.Env):
    ...
    def reset(self, seed=None, state=None):
        if seed is not None:
            ...
            # Also seed the standard random module for consistency
            random.seed(seed)
    ...
    @property
    def np_random(self):
        """Return the seeded numpy random generator"""
        if not hasattr(self, '_np_random') or self._np_random is None:
            # Initialize with a random seed if not already seeded
    ...
    def _rand_int(self, low, high):
        ...
        """
        # Use seeded numpy random generator if available, otherwise fall back to random module
        if hasattr(self, '_np_random') and self._np_random is not None:
            # Handle both legacy RandomState and modern Generator
            if hasattr(self._np_random, 'integers'):
                # Modern numpy.random.Generator (numpy >= 1.17)
                return self._np_random.integers(low, high + 1)
            else:
                # Legacy numpy.random.RandomState
                return self._np_random.randint(low, high + 1)
