...
# 출력 디렉토리 직접 지정
python train_marllib_self/new_compare_visual.py --checkpoint <path> --output-dir ./my_gifs/
"""
    ...
    if 'worker' in checkpoint_data:
        ...
        if 'state' in worker_data:
            ...
            for policy_name in ['helicopter_policy', 'truck_policy', 'crew_policy']:
                if policy_name in state:
                    policy_state = state[policy_name]
                    if 'weights' in policy_state:
                        policies_weights[policy_name] = policy_state['weights']
                        print(f"  ✓ {policy_name} weights 발견")
                    else:
                        print(f"  ⚠ {policy_name} 상태는 있지만 weights 없음")
...
# [새로운 코드 - 이질적 정책 지원]
# ============================================================================
def get_agent_policy_name(agent_id, num_helicopters, num_trucks, num_crews):
    """
    에이전트 ID로부터 정책 이름 반환
...
def main(checkpoint_path, num_episodes=3, seed=42, output_dir=None):
    ...
    # ============================================================================
    # 정책 매핑 함수
    policy_mapping_fn = lambda agent_id: get_agent_policy_name(
        agent_id, num_helicopters, num_trucks, num_crews
    )
            ...
            if 'shared' in policies_weights:
                ...
                for policy_name in ['helicopter_policy', 'truck_policy', 'crew_policy']:
                    if policy_name in policies_weights:
                        ...
                        try:
                            policy_networks[policy_name].load_state_dict(
                                policies_weights[policy_name], strict=False
                            )
                            print(f"  ✓ {policy_name} weights 로드 완료!")
                        except Exception as e:
                            print(f"  ⚠ {policy_name} 로드 실패: {e}")
                            policy_networks.pop(policy_name)
