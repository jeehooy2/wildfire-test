...
def run_episode_and_render(algo, env_config, seed, policy_type="trained"):
    ...
    while not done:
        if policy_type == "trained" and algo is not None:
            ...
            # 첫 번째 에이전트의 정책을 확인하여 shared_policy인지 다중 정책인지 판단
            first_policy_id = policy_mapping_fn(0)

            # 사용 가능한 정책들 확인
            available_policies = list(algo.workers.local_worker().policy_map.keys())

            # shared_policy가 있는 경우 (모든 에이전트가 같은 정책 사용)
            if "shared_policy" in available_policies:
                ...
                # 다중 정책 사용 (각 에이전트가 다른 정책 사용)
                actions = {}
                for agent_id in range(env.num_agents):
                    # 정책 이름 가져오기
                    policy_id = policy_mapping_fn(agent_id)

                    # 해당 정책의 RLModule 가져오기
...
def main(checkpoint_path, num_episodes=10, seed=42, output_dir=None):
    ...
    # Ray 초기화 (train_rllib_mappo_new.py와 동일한 설정)
