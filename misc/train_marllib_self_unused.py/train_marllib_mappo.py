...
실행 예시:
# 기본 실행
python train_marllib_self/train_marllib_mappo.py

# 파라미터 조정 실행
python train_marllib_self/train_marllib_mappo.py --timesteps 10000000 --checkpoint-freq 100
"""
...
from wildfire_marllib_wrapper import RLlibWildfireEnv
...
def train_marllib_mappo(
    ...
    exp_name : str
        실험 이름 (None이면 타임스탬프로 자동 생성)
    """

    # 실험 이름 설정
    if exp_name is None:
        exp_name = f"mappo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    # 실험 저장 디렉토리 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
    local_dir = os.path.join(script_dir, "experiments")
    ...
    print(f"저장 경로: {os.path.join(local_dir, exp_name)}")
    ...
    ENV_REGISTRY["wildfire-ma"] = RLlibWildfireEnv
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="사용할 GPU 수 (기본값: 1, 0이면 CPU만 사용)"
    )
    parser.add_argument(
        "--num-workers",
        type=int,
