...
class AgentState(enum.IntEnum):
    """Agent states for activity and recharging (Phase 4)"""
    ACTIVE = 0
    RETURNING = 1
    RECHARGING = 2
...
class Agent(WorldObj):
    ...
    def __init__(
        ...
        self.target_pos = None  # Target position for grid-based actions

        # Phase 3: Activity time constraints
        self.home_pos = None  # Home base position (set when placed)
        self.max_active_time = 150  # Maximum time agent can be active (increased from 50)
        self.active_time_remaining = self.max_active_time  # Current remaining time

        # Phase 4: Recharging mechanism
        self.state = AgentState.ACTIVE  # Current state
        self.recharge_time = 20  # Time needed to recharge
        self.recharge_time_remaining = 0  # Current recharge progress

        # Water/suppressant system for fire fighting
        self.max_water = 100.0  # Maximum water capacity
        self.water_remaining = self.max_water  # Current water amount
        self.water_consumption_rate = 2.0  # Water consumed per step when on fire
...
class UAVAgent(Agent):
    """
    Unmanned Aerial Vehicle (UAV) agent for wildfire management.
    UAVs have enhanced view range and can fly over obstacles.
    """
    ...
    def __init__(
        ...
        index : int, optional
            a number useful to identify the instantiated agent, by default 0
        view_size : int, optional
            the size of agent view (larger for UAVs), by default 10
        actions : Type[ActionsT], optional
            set of actions available to the agent, by default WildfireActions
        dir_to_vec : list[NDArray], optional
            map of agent direction indices to vectors, by default DIR_TO_VEC
        color : str | None, optional
            color of the agent, by default None
        ...
        """
        super().__init__(
            world, index, view_size, actions, dir_to_vec, color, bg_color, type="uav"
        )
        self.can_fly = True
        self.fire_suppression_effectiveness = 1.0

    def render(self, img):
        """Render the UAV agent with a circular shape
        ...
        """
        c = self.world.COLORS[self.color]
        # Render UAV as a smaller triangle with a circle to represent aerial vehicle
        tri_fn = point_in_triangle(
            (0.15, 0.25),
            (0.85, 0.50),
            (0.15, 0.75),
...
class VehicleAgent(Agent):
    """
    Ground vehicle agent for wildfire management.
    Vehicles have standard view range and enhanced carrying capacity.
    """
    ...
    def __init__(
        ...
        index : int, optional
            a number useful to identify the instantiated agent, by default 0
        view_size : int, optional
            the size of agent view, by default 7
        actions : Type[ActionsT], optional
            set of actions available to the agent, by default WildfireActions
        dir_to_vec : list[NDArray], optional
            map of agent direction indices to vectors, by default DIR_TO_VEC
        ...
        """
        super().__init__(
            world, index, view_size, actions, dir_to_vec, color, bg_color, type="vehicle"
        )
        self.can_fly = False
        self.fire_suppression_effectiveness = 1.2
        self.carrying_capacity = 3

    def render(self, img):
        """Render the vehicle agent with a rectangular shape
        ...
        """
        c = self.world.COLORS[self.color]
        # Render vehicle as a larger, more robust triangle
        tri_fn = point_in_triangle(
            (0.10, 0.15),
            (0.90, 0.50),
            (0.10, 0.85),
...
class CrewAgent(Agent):
    """
    Human crew agent for wildfire management.
    Crew members have standard view range and enhanced fire suppression effectiveness.
    """
    ...
    def __init__(
        ...
        """
        super().__init__(
            world, index, view_size, actions, dir_to_vec, color, bg_color, type="crew"
        )
        self.can_fly = False
        self.fire_suppression_effectiveness = 1.5
        self.carrying_capacity = 1

    def render(self, img):
        """Render the crew agent with a person-like shape
        ...
        """
        c = self.world.COLORS[self.color]
        # Render crew as a standard triangle (same as base agent)
        tri_fn = point_in_triangle(
            (0.12, 0.19),
            (0.87, 0.50),
            (0.12, 0.81),
